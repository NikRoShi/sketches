...
;	../../libs/stm8_GPIO.c: 21: }
	...
	pop	a
	jp	(x)
;	../../libs/stm8_GPIO.c: 23: void writePin(volatile uint8_t* port, uint8_t pin, uint8_t state) {
;	-----------------------------------------
;	 function writePin
;	-----------------------------------------
_writePin:
	sub	sp, #4
;	../../libs/stm8_GPIO.c: 25: *port |= (1 << pin);
	ldw	(0x03, sp), x
	push	a
	ld	a, #0x01
	ld	(0x02, sp), a
...
00114$:
	ld	a, (x)
	ld	(0x02, sp), a
;	../../libs/stm8_GPIO.c: 24: if (state == HIGH) {
	ld	a, (0x07, sp)
	dec	a
	jrne	00102$
;	../../libs/stm8_GPIO.c: 25: *port |= (1 << pin);
	...
	jra	00104$
00102$:
;	../../libs/stm8_GPIO.c: 27: else {*port &= ~(1 << pin);}
	ld	a, (0x01, sp)
	cpl	a
	and	a, (0x02, sp)
	ld	(x), a
...
;	../../libs/stm8_GPIO.c: 28: }
	...
	pop	a
	jp	(x)
;	../../libs/stm8_GPIO.c: 30: void togglePin(volatile uint8_t* port, uint8_t pin) {
;	-----------------------------------------
;	 function togglePin
;	-----------------------------------------
_togglePin:
	...
	ldw	(0x03, sp), x
	ld	(0x02, sp), a
;	../../libs/stm8_GPIO.c: 31: *port ^= (1 << pin);
	ldw	x, (0x03, sp)
	ld	a, (x)
	push	a
	ld	a, #0x01
...
;	../../libs/stm8_GPIO.c: 32: }
	addw	sp, #4
	ret
;	../../libs/stm8_GPIO.c: 34: uint8_t readPin(volatile uint8_t* port, uint8_t pin) {
;	-----------------------------------------
;	 function readPin
;	-----------------------------------------
_readPin:
	...
	ldw	(0x06, sp), x
	ld	(0x05, sp), a
;	../../libs/stm8_GPIO.c: 35: if (*(port +1) & (1 << pin)) {
	ldw	x, (0x06, sp)
	ld	a, (0x1, x)
	push	a
	clrw	x
...
00113$:
	...
	ldw	x, (0x03, sp)
	jreq	00102$
;	../../libs/stm8_GPIO.c: 36: return 1;
	ld	a, #0x01
;	../../libs/stm8_GPIO.c: 38: return 0;
	.byte 0x21
00102$:
	clr	a
00103$:
